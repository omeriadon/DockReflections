...
final class DockSettings {
    ...
    var reflectionEnabled: Bool { didSet { write(reflectionEnabled, for: .enabled) } }
    var reflectionScale: Double { didSet { write(reflectionScale, for: .reflectionScale) } }
    var reflectionYOffset: Double { didSet { write(reflectionYOffset, for: .reflectionYOffset) } }
    var reflectionOpacity: Double { didSet { write(reflectionOpacity, for: .reflectionOpacity) } }
    var reflectionBlurRadius: Double { didSet { write(reflectionBlurRadius, for: .reflectionBlurRadius) } }
    var reflectFolders: Bool { didSet { write(reflectFolders, for: .reflectFolders) } }
    var reflectTrash: Bool { didSet { write(reflectTrash, for: .reflectTrash) } }

    var indicatorsEnabled: Bool { didSet { write(indicatorsEnabled, for: .indicatorsEnabled) } }
    var indicatorWidth: Double { didSet { write(indicatorWidth, for: .indicatorWidth) } }
    var indicatorHeight: Double { didSet { write(indicatorHeight, for: .indicatorHeight) } }
    var indicatorCornerRadius: Double { didSet { write(indicatorCornerRadius, for: .indicatorCornerRadius) } }
    var indicatorYOffset: Double { didSet { write(indicatorYOffset, for: .indicatorYOffset) } }
    var indicatorOpacity: Double { didSet { write(indicatorOpacity, for: .indicatorOpacity) } }
    var indicatorBlurRadius: Double { didSet { write(indicatorBlurRadius, for: .indicatorBlurRadius) } }
    var indicatorTransitionBlurRadius: Double { didSet { write(indicatorTransitionBlurRadius, for: .indicatorTransitionBlurRadius) } }
    var indicatorGlowOpacity: Double { didSet { write(indicatorGlowOpacity, for: .indicatorGlowOpacity) } }
    var indicatorGlowLayers: Int { didSet { write(indicatorGlowLayers, for: .indicatorGlowLayers) } }
    var indicatorEntryDuration: Double { didSet { write(indicatorEntryDuration, for: .indicatorEntryDuration) } }
    var indicatorExitDuration: Double { didSet { write(indicatorExitDuration, for: .indicatorExitDuration) } }
